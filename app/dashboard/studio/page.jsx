...
// ── Main Studio Component (wrapped for Suspense) ───────────
function StudioContent() {
  ...
  // Right panel tab
  const [rightTab, setRightTab] = useState('avatar') // 'avatar' | 'voice' | 'layout'

  // Layout
  const [layout, setLayout] = useState('original') // 'original' | 'circle'
      ...
      if (profile?.heygen_key) {
        ...
        try {
          ...
          if (profile.default_avatar_id) {
            const def = data.avatars?.find(a => a.avatar_id === profile.default_avatar_id)
            if (def) setSelectedAvatar(def)
          } else if (data.avatars?.length) {
            setSelectedAvatar(data.avatars[0])
          }
          if (profile.default_voice_id) {
            const defV = data.voices?.find(v => v.voice_id === profile.default_voice_id)
            if (defV) setSelectedVoice(defV)
          } else if (data.voices?.length) {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col bg-[#0c0c0c] overflow-hidden">
            ...
            <div className="relative w-full max-w-[720px]" style={{ aspectRatio: '16/9' }}>
                ...
                <div className={`w-full h-full bg-[#111] border border-[#1e1e1e] flex flex-col items-center justify-center gap-4 ${layout === 'circle' ? 'rounded-full' : 'rounded-2xl'}`}>
                  {loadingAssets ? (
                    <>
                      ...
                      <p className="text-[12px] text-[#444]">Chargement des avatars...</p>
                    </>
                  ) : (
                    <>
                      <div className="w-16 h-16 rounded-2xl bg-[#1a1a1a] border border-[#222] flex items-center justify-center text-[#2a2a2a]">
                        {I.user}
                      </div>
                      <div className="text-center">
                        <p className="text-[13px] text-[#444] mb-1">Aucun avatar sélectionné</p>
                        <p className="text-[11px] text-[#2a2a2a]">Choisis un avatar dans le panneau de droite</p>
                      </div>
                    </>
                  )}
                </div>
              ...
              {selectedAvatar && (
                <div className="absolute top-3 left-3 flex items-center gap-2 bg-black/60 backdrop-blur-sm border border-white/10 px-2.5 py-1.5 rounded-lg">
                  {selectedAvatar.preview_image_url && (
                    <img src={selectedAvatar.preview_image_url} alt="" className="w-5 h-5 rounded-full object-cover object-top" />
                  )}
          ...
          <div className="border-t border-[#141414] px-6 py-3 flex items-center gap-4">
            <button className="w-7 h-7 rounded-lg bg-[#c0392b] flex items-center justify-center text-white hover:bg-[#a93226] transition flex-shrink-0">
              {I.play}
            </button>
            <div className="flex-1 h-1 bg-[#1a1a1a] rounded-full overflow-hidden">
              <div className="h-full w-0 bg-[#c0392b] rounded-full" />
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="w-8 h-8 rounded-md border border-dashed border-[#1e1e1e] hover:border-[#2a2a2a] flex items-center justify-center text-[#2a2a2a] hover:text-[#555] transition">
                {I.plus}
              </button>
            </div>
          </div>
        ...
        <div className="w-[280px] flex-shrink-0 border-l border-[#1a1a1a] flex flex-col bg-[#090909]">
          ...
          {/* Right panel header with tab icons */}
                ...
                <div className="space-y-1">
                  {filteredVoices.slice(0, 50).map(voice => (
                    <button key={voice.voice_id} onClick={() => setSelectedVoice(voice)}
                      ...
                      <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${selectedVoice?.voice_id === voice.voice_id ? 'bg-[#c0392b]/20 text-[#c0392b]' : 'bg-[#141414] text-[#333]'}`}>
                        {I.mic}
                      </div>
                      <div className="flex-1 min-w-0">
                  ...
                  {filteredVoices.length === 0 && (
                    <p className="text-[11px] text-[#2a2a2a] text-center py-6">Aucune voix trouvée</p>
                  )}
                </div>
              </div>
            )}
              ...
              <div className="p-3 space-y-4 fade-in">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      { id: 'original', label: 'Original', preview: 'rounded-lg' },
