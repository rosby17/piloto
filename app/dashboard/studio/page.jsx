...
const I = {
  ...
  zap: <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M7 1L2 7h5l-1 4 5-6H6l1-4Z" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round"/></svg>,
}
  ...
  return (
    <button onClick={onSelect}
      ...
      <div className="relative overflow-hidden bg-[#111]" style={{ aspectRatio: '3/4' }}>
        ...
        {selected && (
          <div className="absolute top-1.5 right-1.5 w-4 h-4 rounded-full bg-[#c0392b] flex items-center justify-center">{I.check}</div>
        )}
        <div className="absolute bottom-0 left-0 right-0 px-2 py-1.5">
          <p className={`text-[9px] font-medium truncate ${selected ? 'text-[#ff6b5a]' : 'text-[#bbb]'}`}>{avatar.avatar_name || 'Sans nom'}</p>
        </div>
      </div>
    </button>
  )
...
function StudioContent() {
  ...
  const editVideoIdRef  = useRef(null)
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (storedVideoId) {
        ...
        const { data: vid } = await supabase.from('videos').select('*').eq('id', storedVideoId).single()
        if (vid) {
      ...
      // Charger profil : crédits + défauts
      ...
      // Charger avatars & voix — clé HeyGen gérée côté serveur
  ...
  const handleGenerate = async () => {
    ...
    if (credits <= 0) { setError('Crédits insuffisants'); return }
    setGenerating(true)
    setError('')
    try {
      ...
      if (data.success) {
        ...
        setGenerated(true)
        if (data.creditsRestants !== undefined) setCredits(data.creditsRestants)
        setTimeout(() => router.push('/dashboard'), 2000)
      } else if (data.error === 'credits_insuffisants') {
        setError('Plus de crédits disponibles')
      } else {
  ...
  const saveAsDefault = async () => {
    ...
    const updates = {}
    if (selectedAvatar) { updates.default_avatar_id = selectedAvatar.avatar_id; updates.default_avatar_name = selectedAvatar.avatar_name; setDefaultAvatarId(selectedAvatar.avatar_id) }
    if (selectedVoice)  { updates.default_voice_id  = selectedVoice.voice_id;   updates.default_voice_name  = selectedVoice.name;         setDefaultVoiceId(selectedVoice.voice_id) }
  ...
  const filteredAvatars = avatars.filter(a => !avatarSearch || a.avatar_name?.toLowerCase().includes(avatarSearch.toLowerCase()))
  ...
  const saveDraftNow = async (scriptVal, titleVal, avatarVal, voiceVal) => {
    if (!scriptVal?.trim() || hasGeneratedRef.current) return false
    let uid = userRef.current?.id
    if (!uid) {
      const { data: { user: u } } = await supabase.auth.getUser()
      if (!u) return false
      uid = u.id; userRef.current = u
    }
    try {
      if (editVideoIdRef.current) {
        const { error } = await supabase.from('videos').update({ script: scriptVal, titre: titleVal || 'Brouillon', avatar_id: avatarVal?.avatar_id || null, voice_id: voiceVal?.voice_id || null, statut: 'script_pret' }).eq('id', editVideoIdRef.current)
        if (error) throw error
      } else {
        const { data: rows, error } = await supabase.from('videos').insert({ user_id: uid, script: scriptVal, titre: titleVal || 'Brouillon Piloto Studio', avatar_id: avatarVal?.avatar_id || null, voice_id: voiceVal?.voice_id || null, statut: 'script_pret' }).select('id').single()
        if (error) throw error
        if (rows?.id) editVideoIdRef.current = rows.id
      }
      lastSavedScript.current = scriptVal
      return true
    } catch (e) { console.error('Erreur save draft:', e); return false }
  }

  useEffect(() => {
    if (!script.trim() || hasGeneratedRef.current || script === lastSavedScript.current) return
  ...
  const handleBack = async () => {
    if (hasGeneratedRef.current || !script.trim()) { router.push('/dashboard'); return }
    clearTimeout(autoSaveTimer.current)
    setSavingDraft(true)
    setAutoSaveStatus('saving')
    const ok = await saveDraftNow(script, projectTitle, selectedAvatar, selectedVoice)
  ...
  return (
    <div className="h-screen bg-[#0a0a0a] text-white flex flex-col overflow-hidden" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`${FONTS} * { box-sizing: border-box; } ::-webkit-scrollbar { width: 3px; } ::-webkit-scrollbar-track { background: transparent; } ::-webkit-scrollbar-thumb { background: #2a2a2a; border-radius: 2px; } .fade-in { animation: fadeIn .2s ease forwards; } @keyframes fadeIn { from { opacity:0; transform:translateY(4px); } to { opacity:1; transform:translateY(0); } }`}</style>

      {/* TOP BAR */}
      <header className="h-12 border-b border-[#1a1a1a] flex items-center justify-between px-4 flex-shrink-0 bg-[#080808]">
        <div className="flex items-center gap-3">
          <button onClick={handleBack} className="w-7 h-7 rounded-lg bg-[#141414] hover:bg-[#1e1e1e] border border-[#222] flex items-center justify-center text-[#555] hover:text-white transition">
            {savingDraft ? <div className="w-3 h-3 border border-[#c0392b]/40 border-t-[#c0392b] rounded-full animate-spin" /> : I.arrowLeft}
          </button>
          {isEditMode && <div className="flex items-center gap-1.5 text-[10px] text-[#c0392b] border border-[#c0392b]/30 bg-[#c0392b]/8 px-2 py-1 rounded-lg" style={{ fontFamily: "'DM Mono', monospace" }}>{I.edit} Édition</div>}
          {autoSaveStatus === 'saving' && <div className="flex items-center gap-1.5 text-[10px] text-[#555]" style={{ fontFamily: "'DM Mono', monospace" }}><div className="w-2.5 h-2.5 border border-[#333] border-t-[#666] rounded-full animate-spin" />Sauvegarde...</div>}
          {autoSaveStatus === 'saved'  && <div className="flex items-center gap-1.5 text-[10px] text-emerald-500/70" style={{ fontFamily: "'DM Mono', monospace" }}><svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M2 5l2.5 2.5 3.5-4" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>Brouillon sauvegardé</div>}
          {autoSaveStatus === 'error'  && <div className="flex items-center gap-1.5 text-[10px] text-red-400/70" style={{ fontFamily: "'DM Mono', monospace" }}><svg width="10" height="10" viewBox="0 0 10 10" fill="none"><circle cx="5" cy="5" r="4" stroke="currentColor" strokeWidth="1.1"/><path d="M5 3v2.5M5 7v.5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg>Erreur sauvegarde</div>}
          {editingTitle
            ? <input autoFocus value={projectTitle} onChange={e => setProjectTitle(e.target.value)} onBlur={() => setEditingTitle(false)} onKeyDown={e => e.key === 'Enter' && setEditingTitle(false)} className="bg-[#1a1a1a] border border-[#c0392b]/40 rounded-lg px-2.5 py-1 text-[13px] text-white focus:outline-none w-52" style={{ fontFamily: "'DM Mono', monospace" }} />
            : <button onClick={() => setEditingTitle(true)} className="text-[13px] font-medium text-white hover:text-[#aaa] transition truncate max-w-[200px]" style={{ fontFamily: "'DM Mono', monospace" }}>{projectTitle}</button>
          }
        </div>

        <div className="flex items-center gap-2">
          <span className="text-[11px] text-[#444]" style={{ fontFamily: "'DM Mono', monospace" }}>Brand System</span>
          <div className="w-8 h-4 bg-[#c0392b] rounded-full relative cursor-pointer flex items-center px-0.5"><div className="w-3 h-3 bg-white rounded-full ml-auto" /></div>
        </div>

        <div className="flex items-center gap-2">
          {error && <span className="text-[11px] text-red-400 max-w-[200px] truncate">{error}</span>}
          {defaultSaved && <div className="flex items-center gap-1.5 text-[11px] text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1.5 rounded-lg">{I.check} Défauts sauvegardés</div>}

          {/* Badge crédits */}
          {credits !== null && (
            <div className={`flex items-center gap-1.5 text-[11px] px-2.5 py-1.5 rounded-lg border ${
              credits > 1 ? 'text-[#888] border-[#1e1e1e] bg-[#111]'
              : credits === 1 ? 'text-amber-400 border-amber-500/30 bg-amber-500/10'
              : 'text-red-400 border-red-500/30 bg-red-500/10'
            }`} style={{ fontFamily: "'DM Mono', monospace" }}>
              {I.zap} {credits} crédit{credits > 1 ? 's' : ''}
            </div>
          )}

          {generated && <div className="flex items-center gap-1.5 text-[11px] text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1.5 rounded-lg">{I.check} Envoyé à Heygen</div>}
          ...
          <button onClick={handleGenerate} disabled={generating || generated || !selectedAvatar || !selectedVoice || !script.trim() || credits <= 0}
            ...
            {generating ? <><div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Génération...</>
              : generated ? <>{I.check} Généré</>
              : <>{I.spark} Générer</>}
          </button>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-[320px] flex-shrink-0 border-r border-[#1a1a1a] flex flex-col bg-[#090909]">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#141414]">
            ...
            <div className="flex items-center gap-2 text-[10px] text-[#333]" style={{ fontFamily: "'DM Mono', monospace" }}>{wordCount > 0 && <span>{wordCount} mots · ~{estimatedMin} min</span>}</div>
          </div>
          <div className="flex-1 overflow-y-auto">
            <textarea value={script} onChange={e => setScript(e.target.value)} placeholder="Collez ou écrivez votre script ici..."
        ...
        <div className="flex-1 flex flex-col bg-[#0c0c0c] overflow-hidden">
              ...
              {selectedAvatar?.preview_image_url ? (
                <div className={`w-full h-full overflow-hidden bg-[#111] ${layout === 'circle' ? 'rounded-full' : 'rounded-2xl'}`}>
                  ...
                  <div className="absolute bottom-8 left-0 right-0 flex justify-center pointer-events-none">
                    <div className="bg-black/70 backdrop-blur-sm px-5 py-2.5 rounded-xl">
                      <p className="text-white text-[16px] font-semibold text-center">{script.split(' ').slice(0, 5).join(' ')}{script.split(' ').length > 5 ? '...' : ''}</p>
                    </div>
                  </div>
                </div>
              ) : (
                <div className={`w-full h-full bg-[#111] border border-[#1e1e1e] flex flex-col items-center justify-center gap-4 ${layout === 'circle' ? 'rounded-full' : 'rounded-2xl'}`}>
                  {loadingAssets
                    ...
                    : <><div className="w-16 h-16 rounded-2xl bg-[#1a1a1a] border border-[#222] flex items-center justify-center text-[#2a2a2a]">{I.user}</div><div className="text-center"><p className="text-[13px] text-[#444] mb-1">Aucun avatar sélectionné</p><p className="text-[11px] text-[#2a2a2a]">Choisis un avatar dans le panneau de droite</p></div></>}
                </div>
              )}
              {selectedAvatar && (
          ...
          <div className="border-t border-[#141414] px-6 py-3 flex items-center gap-4">
            ...
            <span className="text-[11px] text-[#333]" style={{ fontFamily: "'DM Mono', monospace" }}>00:00 / {estimatedMin > 0 ? `${String(estimatedMin).padStart(2,'0')}:00` : '00:00'}</span>
            <div className="flex items-center gap-2">
              <div className="w-14 h-8 rounded-md border border-[#c0392b]/40 overflow-hidden bg-[#111] flex-shrink-0 relative">
                ...
                <div className="absolute bottom-0.5 right-0.5 bg-[#c0392b] text-white text-[7px] px-1 rounded" style={{ fontFamily: "'DM Mono', monospace" }}>{estimatedMin > 0 ? `${estimatedMin}m` : '--'}</div>
              </div>
        ...
        <div className="w-[280px] flex-shrink-0 border-l border-[#1a1a1a] flex flex-col bg-[#090909]">
          <div className="flex items-center border-b border-[#141414] px-2 py-2 gap-1">
            {[{ id: 'avatar', label: 'Avatar', icon: I.user }, { id: 'voice', label: 'Voix', icon: I.mic }, { id: 'layout', label: 'Layout', icon: I.layout }, { id: 'caption', label: 'Captions', icon: I.captions }].map(tab => (
              <button key={tab.id} onClick={() => setRightTab(tab.id)}
                className={`flex-1 flex flex-col items-center gap-1 py-2 px-1 rounded-lg transition text-[9px] font-medium tracking-wide ${rightTab === tab.id ? 'bg-[#1a1a1a] text-[#c0392b]' : 'text-[#333] hover:text-[#666] hover:bg-[#111]'}`}
                style={{ fontFamily: "'DM Mono', monospace" }}>
                <span className={rightTab === tab.id ? 'text-[#c0392b]' : 'text-current'}>{tab.icon}</span>{tab.label}
              </button>
            ))}
          </div>
            ...
            {rightTab === 'avatar' && (
              <div className="p-3 space-y-3 fade-in">
                  ...
                  <div className="bg-[#111] border border-[#c0392b]/20 rounded-xl overflow-hidden">
                    <div className="flex items-center gap-3 px-3 py-2.5">
                      <div className="w-10 h-10 rounded-lg overflow-hidden bg-[#1a1a1a] flex-shrink-0">
                        {selectedAvatar.preview_image_url ? <img src={selectedAvatar.preview_image_url} alt="" className="w-full h-full object-cover object-top" /> : <div className="w-full h-full flex items-center justify-center text-[#333]">{I.user}</div>}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          <p className="text-[12px] font-medium text-white truncate">{selectedAvatar.avatar_name}</p>
                          {defaultAvatarId === selectedAvatar.avatar_id && <span className="text-[9px] text-[#c0392b] border border-[#c0392b]/30 px-1.5 py-0.5 rounded font-medium flex-shrink-0" style={{ fontFamily: "'DM Mono', monospace" }}>DÉFAUT</span>}
                        </div>
                        <p className="text-[10px] text-[#444]" style={{ fontFamily: "'DM Mono', monospace" }}>{selectedAvatar.type === 'personal' ? 'Mon avatar' : 'Bibliothèque'}</p>
                      </div>
                    ...
                    {defaultAvatarId !== selectedAvatar.avatar_id && (
                      <button onClick={saveAsDefault} disabled={savingDefault} className="w-full flex items-center justify-center gap-1.5 py-2 border-t border-[#1a1a1a] text-[10px] text-[#555] hover:text-[#c0392b] hover:bg-[#c0392b]/5 transition disabled:opacity-40" style={{ fontFamily: "'DM Mono', monospace" }}>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={avatarSearch} onChange={e => setAvatarSearch(e.target.value)} placeholder="Rechercher..." className="w-full bg-[#111] border border-[#1e1e1e] rounded-lg pl-8 pr-3 py-2 text-[12px] text-white placeholder-[#2a2a2a] focus:outline-none focus:border-[#2a2a2a]" />
                </div>
                {loadingAssets ? (
                  <div className="flex items-center justify-center py-8 gap-2 text-[#333]"><div className="w-4 h-4 border-2 border-[#c0392b]/30 border-t-[#c0392b] rounded-full animate-spin" /><span className="text-[11px]">Chargement...</span></div>
                ) : (
                  <>
                    {filteredAvatars.filter(a => a.type === 'personal').length > 0 && (
                      <div>
                        ...
                        <div className="grid grid-cols-3 gap-1.5">{filteredAvatars.filter(a => a.type === 'personal').map(avatar => <AvatarCardMini key={avatar.avatar_id} avatar={avatar} selected={selectedAvatar?.avatar_id === avatar.avatar_id} onSelect={() => setSelectedAvatar(avatar)} />)}</div>
                      </div>
                    )}
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-1.5">
                        {filteredAvatars.filter(a => a.type !== 'personal').map(avatar => (
                          <div key={avatar.avatar_id} className="relative">
                            <AvatarCardMini avatar={avatar} selected={selectedAvatar?.avatar_id === avatar.avatar_id} onSelect={() => setSelectedAvatar(avatar)} />
                            {defaultAvatarId === avatar.avatar_id && <div className="absolute bottom-5 left-0 right-0 flex justify-center pointer-events-none"><span className="text-[7px] text-[#c0392b] bg-black/80 px-1 py-0.5 rounded" style={{ fontFamily: "'DM Mono', monospace" }}>défaut</span></div>}
                          </div>
                        ))}
                      </div>
                      {filteredAvatars.filter(a => a.type !== 'personal').length === 0 && !loadingAssets && <p className="text-[11px] text-[#2a2a2a] text-center py-4">Aucun avatar trouvé</p>}
                    </div>
                  </>
                )}
              </div>
              ...
              <div className="p-3 space-y-3 fade-in">
                    ...
                    <div className="flex items-center gap-3 px-3 py-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          {defaultVoiceId === selectedVoice.voice_id && <span className="text-[9px] text-[#c0392b] border border-[#c0392b]/30 px-1.5 py-0.5 rounded font-medium flex-shrink-0" style={{ fontFamily: "'DM Mono', monospace" }}>DÉFAUT</span>}
                        </div>
                ...
                <div className="flex gap-1 flex-wrap">
                  {[{k:'fr',l:'🇫🇷 FR'},{k:'en',l:'🇺🇸 EN'},{k:'es',l:'🇪🇸 ES'},{k:'all',l:'🌍'}].map(f => (
                    <button key={f.k} onClick={() => setVoiceFilter(f.k)} className={`text-[10px] px-2 py-1 rounded-lg border transition ${voiceFilter === f.k ? 'border-[#c0392b] bg-[#c0392b]/10 text-[#c0392b]' : 'border-[#1a1a1a] text-[#444] hover:border-[#222]'}`} style={{ fontFamily: "'DM Mono', monospace" }}>{f.l}</button>
                  ))}
                </div>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#333]">{I.search}</span>
                  <input type="text" value={voiceSearch} onChange={e => setVoiceSearch(e.target.value)} placeholder="Rechercher une voix..." className="w-full bg-[#111] border border-[#1e1e1e] rounded-lg pl-8 pr-3 py-2 text-[12px] text-white placeholder-[#2a2a2a] focus:outline-none focus:border-[#2a2a2a]" />
                </div>
                <div className="space-y-1">
                  {filteredVoices.slice(0, 50).map(voice => (
                    <button key={voice.voice_id} onClick={() => setSelectedVoice(voice)}
                      className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl border text-left transition ${selectedVoice?.voice_id === voice.voice_id ? 'border-[#c0392b] bg-[#c0392b]/8' : 'border-[#141414] bg-[#0d0d0d] hover:border-[#1e1e1e]'}`}>
                      ...
                      {selectedVoice?.voice_id === voice.voice_id && <div className="w-3.5 h-3.5 rounded-full bg-[#c0392b] flex items-center justify-center flex-shrink-0">{I.check}</div>}
                    </button>
                  ))}
              ...
              <div className="p-3 space-y-4 fade-in">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[{ id: 'original', label: 'Original', preview: 'rounded-lg' }, { id: 'circle', label: 'Circle', preview: 'rounded-full' }].map(l => (
                      <button key={l.id} onClick={() => setLayout(l.id)} className={`flex flex-col items-center gap-2 p-3 rounded-xl border transition ${layout === l.id ? 'border-[#c0392b] bg-[#c0392b]/8' : 'border-[#1a1a1a] bg-[#0d0d0d] hover:border-[#2a2a2a]'}`}>
                        <div className={`w-12 h-8 bg-[#1a1a1a] overflow-hidden ${l.preview} border ${layout === l.id ? 'border-[#c0392b]/40' : 'border-[#222]'}`}>{selectedAvatar?.preview_image_url && <img src={selectedAvatar.preview_image_url} alt="" className="w-full h-full object-cover object-top" />}</div>
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between px-3 py-2.5 bg-[#0d0d0d] border border-[#1a1a1a] rounded-xl">
                    <div className="flex items-center gap-2"><div className="w-5 h-5 rounded-full bg-amber-400/20 flex items-center justify-center"><div className="w-2 h-2 rounded-full bg-amber-400" /></div><span className="text-[12px] text-[#aaa]">Avatar III</span></div>
                    {I.chevronDown}
                  </div>
                </div>
              </div>
              ...
              <div className="p-3 space-y-3 fade-in">
                ...
                <div className="space-y-2">
                  {['Défaut', 'Bold', 'Minimal', 'Karaoke'].map(style => (
                    <button key={style} className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl border text-left transition ${style === 'Défaut' ? 'border-[#c0392b] bg-[#c0392b]/8 text-white' : 'border-[#141414] bg-[#0d0d0d] text-[#555] hover:border-[#1e1e1e]'}`}>
          ...
          <div className="border-t border-[#141414] px-3 py-3">
            {credits !== null && credits <= 0 ? (
              <div className="flex items-center gap-2 px-3 py-2 bg-red-500/8 border border-red-500/20 rounded-xl">
                ...
                <p className="text-[10px] text-red-400" style={{ fontFamily: "'DM Mono', monospace" }}>Crédits épuisés</p>
              </div>
            ) : !selectedAvatar || !selectedVoice ? (
              <p className="text-[10px] text-[#333] text-center" style={{ fontFamily: "'DM Mono', monospace" }}>{!selectedAvatar ? 'Sélectionne un avatar' : 'Sélectionne une voix'}</p>
            ) : (
...
export default function PilotoStudio() {
  return (
    <Suspense fallback={<div className="h-screen bg-[#0a0a0a] flex items-center justify-center"><div className="w-6 h-6 border-2 border-[#c0392b]/30 border-t-[#c0392b] rounded-full animate-spin" /></div>}>
      <StudioContent />
    </Suspense>
  )
}